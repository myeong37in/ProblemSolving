/*
BFS인 것 같긴 한데 칸 중복 방문이 된다. 

하지만 칸을 완전히 똑같은 상태로 중복 방문하는 건 가지치기를 해야겠지.

행, 열, 현재 피스, 다음 숫자를 상태로 두면?
10 * 10 * 3 * 100 = 3 * 10^4
*/
...
std::vector<std::vector<std::vector<std::vector<int>>>> dists;
...
void bfs(int sr, int sc, int N) {
    ...
    q.push({sr, sc, 1, 2, 0});
    q.push({sr, sc, 2, 2, 0});
    dists[sr][sc][0][2] = dists[sr][sc][1][2] = dists[sr][sc][2][2] = 0;

    while (!q.empty()) {
        auto [r, c, piece, next, time] = q.front();
        q.pop();
        ...
        if (piece == 0) { // rook
            ...
            for (int dist = 1; dist <= N; dist++) {
                for (int dir = 0; dir < 4; dir++) {
                    ...
                    if (dists[nr][nc][0][nxt] > time + 1) {
                        dists[nr][nc][0][nxt] = time + 1;

                        q.push({nr, nc, 0, nxt, time + 1});
                    }
                }
            }

            // 교체
            if (dists[r][c][1][next] > time + 1) {
                dists[r][c][1][next] = time + 1;

                q.push({r, c, 1, next, time + 1});
            }

            if (dists[r][c][2][next] > time + 1) {
                dists[r][c][2][next] = time + 1;

                q.push({r, c, 2, next, time + 1});
            }
        }
            ...
            for (int dist = 1; dist <= N; dist++) {
                for (int dir = 0; dir < 4; dir++) {
                    ...
                    if (dists[nr][nc][1][nxt] > time + 1) {
                        dists[nr][nc][1][nxt] = time + 1;

                        q.push({nr, nc, 1, nxt, time + 1});
                    }
                }
            }

            // 교체
            if (dists[r][c][0][next] > time + 1) {
                dists[r][c][0][next] = time + 1;

                q.push({r, c, 0, next, time + 1});
            }
            ...
            for (int dir = 0; dir < 8; dir++) {
                ...
                if (dists[nr][nc][2][nxt] > time + 1) {
                    dists[nr][nc][2][nxt] = time + 1;

                    q.push({nr, nc, 2, nxt, time + 1});
                }
            }
